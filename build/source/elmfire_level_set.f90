...
INTEGER :: I, ILOC, J, IX, IY, ITIMESTEP, IX_IGN, IY_IGN, ISTEP, K, LU, IT1, IT2, &
           ...
           N_TO_TAG, N_SPOT_FIRES, ILH, IT2_LSP, IFBFM, IBLDGFM, ICOL, IROW, YEAR, MONTH, DAY_OF_MONTH, HOUR, BAND_L, BAND_H, IERR=0
...
REAL :: T_LAST_EXTENDED_ATTACK, T_LAST_INTERPOLATE_M1, T_LAST_INTERPOLATE_M10, T_LAST_INTERPOLATE_M100, &
        ...
        T_LAST_SMOKE_OUTPUT, WN_FUEL, RUNTIME, P_IGNITION, & 
...
LOGICAL :: IA_HAS_OCCURRED, LOPEN, GO, CALL_SPOTTING, JUST_INTERPOLATED, DUMP_SMOKE_OUTPUTS, RUN, &
            INITIATED = .FALSE., START_CALCS = .FALSE.
...
DO WHILE (T < NUM_METEOROLOGY_TIMES * DT_METEOROLOGY)
   ...
   IF (T .ge. SIMULATION_TSTART + (IWX_BAND - 1) * DT_METEOROLOGY .and. .not. INITIATED) THEN ! START SIM
      ...
      ! Determine when to dump:
         ...
         DUMPTIMES(:) = 9E9
         DUMPTIMES(0) = 0.
         DO WHILE (T_temp .LE. TSTOP + (IWX_BAND - 1) * DT_METEOROLOGY)
      ...
      IF (.NOT. RANDOM_IGNITIONS) THEN
         ...
         ITLO_METEOROLOGY = IWX_BAND_START
         ITHI_METEOROLOGY = IWX_BAND_START
         F_METEOROLOGY = 1.
      ...
      IF (ENABLE_SMOKE_OUTPUTS .AND. T - T_LAST_SMOKE_OUTPUT .GE. DT_SMOKE_OUTPUTS ) THEN
         ...
         DO I = 1, LIST_BURNED%NUM_NODES
            IF (C%TIME_IGNITED .GE. T_LAST_SMOKE_OUTPUT .AND. C%VELOCITY .GT. 0.01) THEN
               ...
               PM_FLAMING_MAX = PM_EMISSION_FACTOR_FLAMING * WET_WOOD_CALORIFIC_VALUE * CELLENERGYRELEASE
               PM_SMOLDERING_MAX = 0.43 * PM_EMISSION_FACTOR_SMOLDERING * WET_WOOD_CALORIFIC_VALUE * CELLENERGYRELEASE
   ...
   !   ENDIF
      ...
      IF (ABS(T - DUMPTIMES(IDUMPCOUNT)) .LE. 0.5*DT .OR. T .GE. DUMPTIMES(IDUMPCOUNT) .OR. T .GE. TSTOP + (IWX_BAND - 1) * DT_METEOROLOGY) THEN
