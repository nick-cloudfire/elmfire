...
! *****************************************************************************
SUBROUTINE MINIMUM_TRAVEL_TIME(IIGN, ROS, DIR, FLIN, CAN, MTT)
! *****************************************************************************
INTEGER, INTENT(IN) :: IIGN
TYPE(RASTER_TYPE), INTENT(IN) :: ROS, DIR, FLIN, CAN
TYPE(RASTER_TYPE), INTENT(OUT) :: MTT
TYPE(NODE), POINTER :: C
TYPE(DLL) :: LIST_NODES_UP_NEXT
LOGICAL*1, ALLOCATABLE, DIMENSION(:,:) :: IN_QUEUE
INTEGER :: DI, DJ, IX, IY, JX, JY
REAL :: DISTANCE, BEARING, CAND, T_HERE
REAL, PARAMETER :: FT_PER_M = 1. / 0.3048, BIG = 1E30, DEG_TO_RAD = PI / 180.

CALL ALLOCATE_EMPTY_RASTER(MTT, ROS%NCOLS, ROS%NROWS, 1, ROS%XLLCORNER, ROS%YLLCORNER, ROS%CELLSIZE, ROS%NODATA_VALUE, 'FLOAT     ')

ALLOCATE(IN_QUEUE(1:MTT%NCOLS,1:MTT%NROWS))
IN_QUEUE(:,:) = .FALSE.

LIST_NODES_UP_NEXT = NEW_DLL()

IX = ICOL_FROM_X(X_IGN(IIGN),ANALYSIS_XLLCORNER,ANALYSIS_CELLSIZE)
IY = IROW_FROM_Y(Y_IGN(IIGN),ANALYSIS_YLLCORNER,ANALYSIS_CELLSIZE)

IF (IX .LT. 1 .OR. IX .GT. MTT%NCOLS .OR. IY .LT. 1 .OR. IY .GT. MTT%NROWS) THEN
   DEALLOCATE(IN_QUEUE)
   RETURN
ENDIF

MTT%R4(IX,IY,1) = 0.
CALL APPEND(LIST_NODES_UP_NEXT, IX, IY, 0.0_8)
IN_QUEUE(IX,IY) = .TRUE.

DO WHILE (LIST_NODES_UP_NEXT%NUM_NODES .GT. 0)

! Pop the head. DELETE_NODE repositions C, so cache the indices before deleting:
   C => LIST_NODES_UP_NEXT%HEAD
   IX = C%IX
   IY = C%IY
   CALL DELETE_NODE(LIST_NODES_UP_NEXT, C)
   IN_QUEUE(IX,IY) = .FALSE.

   T_HERE = MTT%R4(IX,IY,1)
   IF (T_HERE .EQ. MTT%NODATA_VALUE) CYCLE

   DO DJ = -1, 1
   DO DI = -1, 1
      IF (DI .EQ. 0 .AND. DJ .EQ. 0) CYCLE

! Rasters are indexed (ICOL,IROW,IBAND), so DI offsets the column and DJ the row:
      JX = IX + DI
      JY = IY + DJ
      IF (JX .LT. 1 .OR. JX .GT. MTT%NCOLS) CYCLE
      IF (JY .LT. 1 .OR. JY .GT. MTT%NROWS) CYCLE
      IF (ISNONBURNABLE(JX,JY)) CYCLE

      IF (DI .EQ. 0 .OR. DJ .EQ. 0) THEN
         DISTANCE = MTT%CELLSIZE * FT_PER_M              ! ft
      ELSE
         DISTANCE = MTT%CELLSIZE * FT_PER_M * SQRT(2.)   ! ft
      ENDIF

! Bearing of travel, clockwise from north. Rows increase southward, so the
! northward component of the offset is -DJ, not +DJ:
      BEARING = ATAN2(REAL(DI), REAL(-DJ))

      CAND = T_HERE + TRAVEL_TIME(IX,IY,BEARING,0.5*DISTANCE) &
                    + TRAVEL_TIME(JX,JY,BEARING,0.5*DISTANCE)
      IF (CAND .GE. BIG) CYCLE

      IF (MTT%R4(JX,JY,1) .EQ. MTT%NODATA_VALUE .OR. CAND .LT. MTT%R4(JX,JY,1)) THEN
         MTT%R4(JX,JY,1) = CAND
         IF (.NOT. IN_QUEUE(JX,JY)) THEN
            CALL APPEND(LIST_NODES_UP_NEXT, JX, JY, REAL(CAND,8))
            IN_QUEUE(JX,JY) = .TRUE.
         ENDIF
      ENDIF
   ENDDO
...
! *****************************************************************************
REAL FUNCTION TRAVEL_TIME(IXL, IYL, BEARING, HALFDIST)
! *****************************************************************************
! Minutes needed to cover HALFDIST feet inside cell (IXL,IYL) while spreading
! along compass BEARING (radians). Returns BIG where the cell cannot carry fire.

INTEGER, INTENT(IN) :: IXL, IYL
REAL, INTENT(IN) :: BEARING, HALFDIST
INTEGER :: IFBFM, ILH
REAL :: IR, LOW, S, THETA, CELL_ROS, WSMFEFF
REAL, PARAMETER :: KWPM2_TO_BTUPFT2MIN = 60. * 0.3048 * 0.3048 / 1.055

TRAVEL_TIME = BIG
IF (ROS%R4(IXL,IYL,1) .LE. 0. .OR. ROS%R4(IXL,IYL,1) .EQ. ROS%NODATA_VALUE) RETURN

IFBFM = MAX(MIN(INT(FBFM%I2(IXL,IYL,1)),303),0)
ILH   = MAX(MIN(NINT(100.*MLH%R4(IXL,IYL,1)),120),30)

! NOTE: UX_AND_UY_ELLIPTICAL raises the combined wind-and-slope coefficient
! (PHIMAG) to B_COEFF_INVERSE here, not the gridded wind speed. WS is only a
! stand-in - the effective midflame wind is not recoverable from ROS and DIR
! alone, so this term needs revisiting before the output is trusted.
WSMFEFF = FUEL_MODEL_TABLE_2D(IFBFM,ILH)%WSMFEFF_COEFF * &
          MAX(WS%R4(IXL,IYL,1),0.) ** FUEL_MODEL_TABLE_2D(IFBFM,ILH)%B_COEFF_INVERSE

! CAN <= 0 stands in for "no crown fire here", so the Rothermel wind cap applies.
! IR is recovered by inverting FLIN = TR * IR * ROS * 0.3048:
IF (CAN%R4(IXL,IYL,1) .LE. 0. .AND. FUEL_MODEL_TABLE_2D(IFBFM,ILH)%TR .GT. 0.) THEN
   IR = FLIN%R4(IXL,IYL,1) / (FUEL_MODEL_TABLE_2D(IFBFM,ILH)%TR * ROS%R4(IXL,IYL,1) * 0.3048)
   WSMFEFF = MIN(WSMFEFF, 0.9*KWPM2_TO_BTUPFT2MIN*IR)
ENDIF

LOW = MIN( 0.936*EXP(0.1147*WSMFEFF*WSMFEFF_LOW_MULT) + &
           0.461*EXP(-0.0692*WSMFEFF*WSMFEFF_LOW_MULT) - 0.397, MAX_LOW)

IF (LOW .LE. 1.001) THEN
   CELL_ROS = ROS%R4(IXL,IYL,1)   ! Circular front; also keeps SQRT(LOW^2-1) real
ELSE
   S        = SQRT(LOW*LOW - 1.)
   THETA    = BEARING - DIR%R4(IXL,IYL,1)*DEG_TO_RAD
   CELL_ROS = ROS%R4(IXL,IYL,1) / (LOW*(LOW + S)*(1. - (S/LOW)*COS(THETA)))
ENDIF

IF (CELL_ROS .LE. 0.) RETURN
TRAVEL_TIME = HALFDIST / CELL_ROS

! *****************************************************************************
END FUNCTION TRAVEL_TIME
! *****************************************************************************

! *****************************************************************************
END SUBROUTINE MINIMUM_TRAVEL_TIME
! *****************************************************************************
