...
SUBROUTINE SPOTTING(IX0,IY0,WS20_NOW,FLIN, N_SPOT_FIRES, IX_SPOT_FIRE, IY_SPOT_FIRE, &
                    ICASE, DT, TIME_NOW, TAU, IGNMULT, IFBFM)
! *****************************************************************************
...
REAL, INTENT(IN) :: CELLSIZE_ELM, PIGN_ELM, MIN_SPOTTING_DISTANCE, MAX_SPOTTING_DISTANCE, &
                    SIGMA_DIST, MU_DIST, SIGMA_SPANWISE, MU_SPANWISE, TIME_NOW, IGNMULT, FLIN
REAL, INTENT(IN) :: PHIP(:,:)
...
IF (USE_UMD_SPOTTING_MODEL) THEN
   ...
   LOW  = SARDOY_PDFINV(0.0, MU_DIST, SIGMA_DIST)  !moved these up since they do not need to be recalculated per ember
...
! Get spotting distance
   ...
   if (SPOTTING_DISTANCE .LE. CELLSIZE_ELM) CYCLE !no need to track embers that travel less than one cellsize.
   ...
   SPOTTING_STATS(NUM_TRACKED_EMBERS)%DIST              = SPOTTING_DISTANCE
   ...
   SPOTTING_STATS(NUM_TRACKED_EMBERS)%FLIN              = FLIN
...
! Find the maximum spotting distance accoring to criterion P_EPS (P_EPS=0.01 by default)
IF (USE_UMD_SPOTTING_MODEL) THEN
