...
! Get spotting distance
   ...
   ITLO_METEOROLOGY = 1 + FLOOR((T+TIME_NOW) / DT_METEOROLOGY) - MINIMUM_CURRENT_WX_BAND + 1
   ITLO_METEOROLOGY = MIN(MAX(ITLO_METEOROLOGY,1), WX_BANDS_KEPT_IN_MEM, WS%NBANDS)
   ITHI_METEOROLOGY = MIN(ITLO_METEOROLOGY + 1, WX_BANDS_KEPT_IN_MEM, WS%NBANDS)     
   F_WIND = ((T+TIME_NOW) - REAL(ITLO_METEOROLOGY + MINIMUM_CURRENT_WX_BAND - 2) * DT_METEOROLOGY) / DT_METEOROLOGY
   F_WIND = MIN(MAX(F_WIND, 0.), 1.)
   ...
   IF (T .LT. 1E9) THEN
      ...
      IF (TRIM(IGNITION_MODEL) .EQ. 'DIRECT') THEN
         ...
         IF(IFBFM .NE. 91) THEN
            P_IGNITION = PIGN_ELM
         ELSE
            IF(USE_BLDG_SPREAD_MODEL .AND. BLDG_SPREAD_MODEL_TYPE .NE. 1) THEN
...
ITLO_METEOROLOGY = 1 + FLOOR((C%TIME_ACTUAL) / DT_METEOROLOGY) - MINIMUM_CURRENT_WX_BAND + 1
ITLO_METEOROLOGY = MIN(MAX(ITLO_METEOROLOGY,1), WX_BANDS_KEPT_IN_MEM, WS%NBANDS)
ITHI_METEOROLOGY = MIN(ITLO_METEOROLOGY + 1, WX_BANDS_KEPT_IN_MEM, WS%NBANDS)     
F_WIND = ((C%TIME_ACTUAL) - REAL(ITLO_METEOROLOGY + MINIMUM_CURRENT_WX_BAND - 2) * DT_METEOROLOGY) / DT_METEOROLOGY
F_WIND = MIN(MAX(F_WIND, 0.), 1.)
IF (ITLO_METEOROLOGY .EQ. ITHI_METEOROLOGY) F_WIND = 1.
