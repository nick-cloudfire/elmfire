...
IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(14)) THEN
   if (.not. USE_CFFDRS .and. CBH_FILENAME .ne. ' ') THEN
...
IF (IRANK_WORLD .EQ. PARALLEL_IO_RANK(15)) THEN
   if (.not. USE_CFFDRS .and. CBD_FILENAME .ne. ' ') THEN
...
IF (USE_BARRIERS .AND. IRANK_WORLD .EQ. PARALLEL_IO_RANK(41)) CALL READ_BSQ_RASTER (BARRIER_WIDTH, FUELS_AND_TOPOGRAPHY_DIRECTORY, BARRIER_FILENAME)
...
IF (DUMP_SPREAD_RATE .AND. IDUMPCOUNT .EQ. NDUMPS) THEN
   ...
   if (SPREAD_RATE_IN_M) then
      DO I = 1, LIST_BURNED%NUM_NODES
         RDUMPME%R4(C%IX,C%IY,1) = C%VELOCITY * 0.3048
         C => C%NEXT
      ENDDO
   else
      DO I = 1, LIST_BURNED%NUM_NODES
