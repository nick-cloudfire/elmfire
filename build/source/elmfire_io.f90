...
! below) when it does not already exist. READ_BSQ_XML_HEADER performs the
! equivalent conversion on its own path; without doing it here too, a
! USE_BSQ_XML_HEADER=.FALSE. run reads a .hdr that nothing has created yet and
! dies with 'ERROR OPENING HDR FILE'. Uses the same INQUIRE + PATH_TO_GDAL
! idiom as the landscape-file branch above.
INQUIRE(FILE=TRIM(FNHDR),EXIST=HDR_EXISTS)
INQUIRE(FILE=TRIM(FNBSQ),EXIST=BSQ_EXISTS)
IF (.NOT. USE_EXISTING_BSQS .AND. (.NOT. HDR_EXISTS .OR. .NOT. BSQ_EXISTS)) THEN
   SHELLSTR = TRIM(PATH_TO_GDAL) // 'gdal_translate -q -of ENVI -co "INTERLEAVE=BSQ" ' // TRIM(FNTIF) // " " // TRIM(FNBSQ)
   IF (FEEDBACK_LEVEL .GE. 3) WRITE(*,'(A)') TRIM(SHELLSTR)
   CALL EXECUTE_COMMAND_LINE(TRIM(SHELLSTR))
ENDIF
