...
IF (USE_LANDSCAPE_FILE) THEN
   IF (IRANK_WORLD .EQ. 0) THEN
      ...
      IF (.NOT. HDR_EXISTS .OR. .NOT. BSQ_EXISTS) THEN
         SHELLSTR = GDAL_COMMAND('gdal_translate', '-q -of ENVI -co "INTERLEAVE=BSQ" ' // TRIM(FNTIF) // " " // TRIM(FNBSQ))
...
!      FLUSH(LUOUT)
      ...
      IF (CONVERT_TO_GEOTIFF_LOCAL) THEN
         IF (COMPRESS) THEN
            SHELLSTR = GDAL_COMMAND('gdal_translate', '-q -ot Float32 -co "COMPRESS=DEFLATE" -co "ZLEVEL=9" -a_srs "' // &
                       TRIM(A_SRS) // '" ' // TRIM(FNBIL) // ' ' // TRIM(FNTIF))
            IF (FEEDBACK_LEVEL .GE. 3) WRITE(*,100) TRIM(SHELLSTR)
            CALL EXECUTE_COMMAND_LINE(TRIM(SHELLSTR))
         ELSE
            SHELLSTR = GDAL_COMMAND('gdal_translate', '-q -ot Float32 -a_srs "' // &
                       TRIM(A_SRS) // '" ' // TRIM(FNBIL) // ' '// TRIM(FNTIF))
            IF (FEEDBACK_LEVEL .GE. 3) WRITE(*,100) TRIM(SHELLSTR)
            CALL EXECUTE_COMMAND_LINE(TRIM(SHELLSTR))
         ENDIF

         SHELLSTR = TRIM(DELETECOMMAND) // " " // SHELL_PATH(FNBIL); IF (FEEDBACK_LEVEL .GE. 3) WRITE(*,100) TRIM(SHELLSTR)
         CALL EXECUTE_COMMAND_LINE(TRIM(SHELLSTR))

         SHELLSTR = TRIM(DELETECOMMAND) // " " // SHELL_PATH(FNHDR); IF (FEEDBACK_LEVEL .GE. 3) WRITE(*,100) TRIM(SHELLSTR)
         CALL EXECUTE_COMMAND_LINE(TRIM(SHELLSTR))
   ...
   CASE ('SIGNEDINT') 
      ...
      IF (CONVERT_TO_GEOTIFF_LOCAL) THEN
         IF (COMPRESS) THEN
            SHELLSTR = GDAL_COMMAND('gdal_translate', '-q -ot Int16 -co "COMPRESS=DEFLATE" -co "ZLEVEL=9" -a_srs "' // &
                       TRIM(A_SRS) // '" ' // TRIM(FNBIL) // ' ' // TRIM(FNTIF))
            IF (FEEDBACK_LEVEL .GE. 3) WRITE(*,100) TRIM(SHELLSTR)
            CALL EXECUTE_COMMAND_LINE(TRIM(SHELLSTR))
         ELSE
            SHELLSTR = GDAL_COMMAND('gdal_translate', '-q -ot Int16 -a_srs "' // &
                       TRIM(A_SRS) // '" ' // TRIM(FNBIL) // ' ' // TRIM(FNTIF))
            IF (FEEDBACK_LEVEL .GE. 3) WRITE(*,100) TRIM(SHELLSTR)
            CALL EXECUTE_COMMAND_LINE(TRIM(SHELLSTR))
         ENDIF
...
INTEGER :: I, J, IDUMMY, IOS, IPOS, ITYPE, NLINES
...
IF (DELETE_INTERMEDIATE_FILES) THEN
   SHELLSTR = TRIM(DELETECOMMAND) // " " // SHELL_PATH(FNBSQ) // " " // SHELL_PATH(FNHDR) // " " // &
              SHELL_PATH(FNPRJ) // " " // SHELL_PATH(FNXML)
   IF (FEEDBACK_LEVEL .GE. 3) WRITE(*,100) TRIM(SHELLSTR)
   CALL EXECUTE_COMMAND_LINE(TRIM(SHELLSTR))
ENDIF
